...
// IF there is a CUDA error, print the error message and exit
// Error message contains information about the file and line number where the error occurred
// Macros means that the code is expanded inline where it is used, rather than being a function call
// fprintf is used to print the error message to standard error (stderr)
// we can't use printf because it may not be thread-safe in a CUDA context
// whereas fprintf is safe to use in this context
// Reason fprintf is used instead of std::cerr is because 
// fprintf is a C-style function and is more commonly used in CUDA programming
// whereas printf is a C++-style function and may not be as widely supported in CUDA
// exit(EXIT_FAILURE) is used to terminate the program with a failure status
...
// Timing structure
/// io_time: Time taken to read input matrices A and B
/// l_time: Time taken to compute L matrix
/// u_time: Time taken to compute U matrix
/// total_solve_time: Total time taken to solve the linear equations
typedef struct {
    double io_time;
    double l_time;
    double u_time;
...
// total_threads: Total number of threads used across all kernels
// blocks_per_grid: Number of blocks per grid 
// threads_per_block: Number of threads per block 
// cgma_ratio: Computation to Global Memory Access ratio
// total_flops: Total floating-point operations performed
// total_memory_accesses: Total memory accesses performed (global and shared are included here)
typedef struct {
    int total_threads;
    int blocks_per_grid;
...
// pivot: Pointer to pivot row index
